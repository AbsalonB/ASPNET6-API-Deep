...
#pragma warning restore CS8603 // Possible null reference return.
    ...
    public async Task<PagedList<Author>> GetAuthorsAsync(AuthorResourceParameters authorResourceParameters)
    {
        ...
        if (!string.IsNullOrWhiteSpace(authorResourceParameters.SearchQuery))
        {
            ...
            || a.FirstName.Contains(authorResourceParameters.SearchQuery) || a.LastName.Contains(authorResourceParameters.SearchQuery));
        }
        ...
        if(!string.IsNullOrWhiteSpace(authorResourceParameters.OrderBy))
        {
            ...
            var authorPropertyMapperDictionary = _propertyMappingService.GetPropertyMapping<AuthorDto, Author>();
            collection = collection.ApplySort(authorResourceParameters.OrderBy, authorPropertyMapperDictionary);
        }
